...
export function UpdateStatusModal({ open, onOpenChange, patient, onSave }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await internationalPatient.updateInternationalPatient(
        patient._id,
        updateData,
      );
          ...
          {patient && (
            <div className="bg-gray-50 p-4 rounded-lg">
              ...
              <div className="text-sm text-gray-600">
                <p>
                  <strong>Name:</strong> {patient.patient_full_name}
                </p>
                <p>
                  <strong>Country:</strong> {patient.country}
                </p>
                <p>
                  <strong>Current Status:</strong> {patient.status || "Not set"}
                </p>
              </div>
            </div>
          )}
          ...
          {selectedStatus === "Cancelled" && (
            <div className="space-y-2">
              ...
              <textarea
                ...
                className="w-full min-h-[100px] px-3 py-2 bg-[#FBFBFB] rounded-[6px] border border-[#DDDDDD] text-sm resize-none focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent"
                required
              />
            </div>
          )}
